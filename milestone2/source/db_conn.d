...
struct DBConnection
{
    ...
    UserRet addUser(string email, string username, string password, string name = "", string desc = "")
    {
        ...
        string hashpass = makeHash(toPassword(password.dup)).toString();
    ...
    UserRet authUser(string email, string password)
    {
        ...
        auto pass = res["password"].toString().strip("\"");
        auto saltpass = parseHash(pass).salt;
        auto hashpass = makeHash(toPassword(password.dup), saltpass);
         
        if (pass != hashpass.toString())
        {
